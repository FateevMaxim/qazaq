...
class ProductController extends Controller
{
    public function addChina(Request $request)
    {
        ...
        $array =  preg_split("/\s+/", $request["track_code"]);
        $wordsFromFile = [];
        foreach ($array as $ar){
            $wordsFromFile[] = [
                'track_code' => $ar,
    ...
    public function addHargos(Request $request)
    {
        ...
        foreach ($array as $ar){
            $wordsFromFile[] = [
                'track_code' => $ar,
                'to_hargos' => date(now()),
                'status' => 'Получено в Харгос',
                'updated_at' => date(now()),
            ];
        }
        TrackList::upsert($wordsFromFile, ['track_code', 'to_hargos', 'status', 'updated_at']);
        return response('success');
    ...
    public function getInfoProduct(Request $request)
    {
        ...
        $track_code_statuses =  TrackList::query()->select('to_china', 'to_hargos', 'to_almaty', 'to_client', 'client_accept')->where('track_code', $request['track_code'])->first();
        if ($track_code){
